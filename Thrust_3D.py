...
inputPath = os.path.join(os.path.abspath("."),"LMS256_dep24_edge18_ptcl30_L240/")
outputPath = os.path.join(os.path.abspath("."),"LMS256_dep24_edge18_ptcl30_L240/")
...
minX = nd(   -180.* u.kilometer)
maxX = nd( 60. * u.kilometer)
minY = nd(  -48. * u.kilometer)
maxY = nd( 48. * u.kilometer)
...
fault_lowDip = -nd(24.*u.kilometer)
...
markSwarm1.add_particles_with_coordinates(np.array([[-nd(1.5*u.kilometer)+x_shift+0.5*dx_min,0.,-nd(3*u.kilometer)]]))
markSwarm2.add_particles_with_coordinates(np.array([[-nd(14.5*u.kilometer)+x_shift+0.5*dx_min,0.,-nd(10*u.kilometer)]]))
markSwarm3.add_particles_with_coordinates(np.array([[-nd(14.5*u.kilometer)+x_shift+0.5*dx_min,-nd(22.5*u.kilometer),-nd(10*u.kilometer)]]))

markSwarm4.add_particles_with_coordinates(np.array([[-nd(63*u.kilometer)+x_shift+0.5*dx_min,0.,-nd(28*u.kilometer)]]))
...
if(LoadFromFile == False):
    ...
    faultRange_x0 =  (i0>=minY+edgeGap) & (i0<=maxY-edgeGap)   

    material_map = [(((y>y1-1*dx_min) & (x<x_shift) & (x>x1) & (y<y1) & (y<top_coord) & (y>nd(-30.*u.kilometer)) & faultRange_x0),fault),
    (((x>x1_r) & (x<x1_r+dx_min) & (y<y1) & (y<top_coord) & (y>nd(-30.*u.kilometer)) & faultRange_x0 ),fault),               
    (((y>y1-1*dx_min) & (x<x_shift) & (x>x1) & (y<y1) & (y<top_coord) & (y<=nd(-30.*u.kilometer)) & faultRange_x0 ),faultLC),
    (((x>x1_r) & (x<x1_r+dx_min) & (y<y1) & (y<top_coord) & (y<=nd(-30.*u.kilometer)) & faultRange_x0 ),faultLC),             
    ...
    surfacePoints[:,2] = diff_coord/((x_shift-x1)**2)*(surfacePoints[:,0]-x1)**2+base_coord
    faultSwarm.add_particles_with_coordinates( surfacePoints )

countz = 100
surfacePoints1 = np.zeros((countz,3))
surfacePoints1[:,1] = 0.
surfacePoints1[:,0] = np.linspace(minX,maxX,countz)
surfacePoints1[:,2] = 0.

surfacePoints2 = np.zeros((countz,3))
surfacePoints2[:,1] = -0.225
...
if LoadFromFile == False:    
    ...
    condition_b =      [(coordz>p01.value[2],b_max),
                        (((coordz>-nd(27.*u.kilometer)) & (coordz<= p01.value[2])),b0),
...
freeslipBC = uw.conditions.DirichletCondition( variable        = velocityField, 
                                               indexSetsPerDof = (iWalls, jWalls,base) )
...
#condw = dt_theta *L/VpFn #[(plasticStrain0>0,dt_theta *L_field/VpFn),
...
#             csv_write.writerow(SP_output)
    ...
    if step>15:
        ...
        dt0 = np.min([dt_max,dt_vep,4.*dt_e.value])
        dt_e.value =  np.max([dt0,dt_min,0.25*dt_e.value]) 
